...
Prompt guidance tells the model to keep one current goal plus assumptions and decisions that should affect future behavior, not routine implementation progress. It also tells agents to clean stale board items automatically when scope or goals change, using direct archive for routine stale items and `decision_board.review_cleanup` for ambiguous current-context changes.
